...
- varsayılan akışta hiçbir provider çağrısı yok; canlı smoke/batch yalnız explicit `--live`, non-default config, preflight ve disjoint output ile açılır. Gerçek kaynak egress'i insan-review/selection kapıları geçmeden yapılmaz.
...
Durum: **yerel sözleşme/güvenlik altkümesi ve sınırlı canlı smoke uygulandı.** Field policy, yalnız leaf segment extraction/host merge, sentinel-bütünlüğü request/response doğrulaması, immutable prompt bundle, kayıtlı-response fake provider, pre-egress scan, exact human-promoted memory, not-sent research policy ve atomic MQM/Wilson/human-only acceptance sözleşmesi fixture testleriyle vardır. DeepSeek V4 çevirici ve OpenAI Responses judge, allow-list'li secret resolver, public HTTPS/no-redirect transport ve preflight arkasında sabit sentetik isteklerle doğrulanmıştır. Gerçek kaynak üzerinde `translate` hâlâ kapalıdır; gerçek client/fetch/repair/batch workflow'u kaynak ve insan-review kapıları olmadan açılmaz.
...
Gerçek çeviri/judge API kanıtı yalnız sentetik smoke sınırındadır. Kimlik, veri formatı, immutable publish ve temel tekrar-almama garantileri fixture/property testleriyle doğrulanır; iki gerçek kaynak için tam canonical/karantina muhasebesi modelsiz pilotta tamamlanmıştır.
