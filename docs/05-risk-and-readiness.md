...
Güncel karar: **Faz 1–5 temel, sınırlı canlı provider yüzeyi, iki gerçek kaynağın modelsiz pilotu, karar üretmeyen insan-review worklist'i, pre-review canary ve 50-episode bounded automation regression'ı hazırdır; bu insan review kararı veya release'e geçiş onayı değildir.** Doğrulanmış v0.3 / Flash 6-worker regression 46 translation, 541 accepted leaf ve 40 pending-HF review row üretti. v0.4 aday prompt aynı cohortta bu sonucu geçemediği için production'a alınmadı; candidate/retired prompt egress öncesi reddedilir. Pre-review canary, teknik/prompt/provider testini insan kabulundan önce yapabilir; insan review S400/Gold/release için son karar kapısıdır. Varsayılan config çevrimdışıdır; canlı komutlar explicit `--live`, non-default config, preflight ve disjoint output gerektirir. Aşağıdaki listelerde test/model kanıtı, modelsiz gerçek kaynak kanıtı ve henüz açık üretim koşulları ayrılır.
