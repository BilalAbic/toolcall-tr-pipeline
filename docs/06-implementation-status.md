...
| Source adapterları | Uygulandı | `src/toolcall_tr/adapters/xlam.py`, `xlam60k.py`, `when2call.py` | xLAM fixtureları, xLAM 60k embedded-JSON/APIGen tür dönüşümü ve When2Call adapterı; adapter kaynak davranışını tamir etmez veya icat etmez |
...
| Faz 5 field policy ve host merge | Fixture düzeyinde uygulandı | `field_policy.py`, `configs/field_policy.toml` | Yalnız explicit-policy leaf segmentleri; technical fields immutable, coverage/merge fail-closed |
...
| Operasyonel canlı çeviri | Uygulandı, gerçek kaynak egress'i kapalı | `src/toolcall_tr/operational_translation.py`, `translate` CLI | Yalnız field-policy tarafından izinli leaf'ler DeepSeek'e gider; claim/attempt/leaf result immutable, source rehash ve host merge zorunlu. 3 sentetik no-tool episode'u gerçek endpointte geçti. Otomatik retry, Gold ve release yok. |
...
1. When2Call ve xLAM 60k karantinalarını ve xLAM'daki 6 hard conflict'i, kaynak kayıtlarını değiştirmeden yetkili insan review/adjudication ile ele al. Quarantine/conflict varsa karar olmadan devam etme.
