...
def test_checked_in_prompt_bundle_is_content_addressed_and_has_no_secret_surface() -> None:
    ...
    assert bundle.prompt_version == "translation-prompt-0.3.0"
    assert bundle.promotion_status == "validated"
...
def test_candidate_and_unknown_top_level_prompt_metadata_are_rejected_for_live_use(
    tmp_path: Path,
) -> None:
    from toolcall_tr.prompt_contract import require_validated_prompt

    candidate = build_prompt_bundle(
        prompt_version="candidate-1",
        promotion_status="candidate",
        layers=layers(),
    )
    with pytest.raises(PromptContractError, match="promotion_status=validated"):
        require_validated_prompt(candidate)

    unknown = tmp_path / "unknown-top-level.toml"
    unknown.write_text(
        'prompt_version = "test"\npromotion_status = "validated"\nunsafe = true\n[layers]\n',
        encoding="utf-8",
    )
    with pytest.raises(PromptContractError, match="unknown top-level"):
        load_prompt_bundle(unknown)
